export function generatePDFHtml({ user, contas, cartoes }) {
  ...
  const saldoFinal =
    totalReceitas - (totalDespesasContas + totalDespesasCartoes);
  ...
  return `
      <html>
        ...
        <body>
          ...
          <h3>Total Geral de Despesas (Contas): ${formatMoney(
            totalDespesasContas
          )}</h3>
          <h3>Total Geral de Despesas (Cartões): ${formatMoney(
            totalDespesasCartoes
          )}</h3>
